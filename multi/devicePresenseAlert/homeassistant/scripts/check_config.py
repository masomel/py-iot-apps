...
def run(script_args: List) -> int:
    ...
    if len(res['except']) > 0:
        ...
        for domain, config in list(res['except'].items()):
    ...
    if domain_info:
        if 'all' in domain_info:
            ...
            for domain, config in list(res['components'].items()):
    ...
    if args.secrets:
        ...
        for sfn, sdict in list(res['secret_cache'].items()):
            sss = []
            for skey, sval in list(sdict.items()):
        ...
        for skey, sval in list(res['secrets'].items()):
...
def check(config_path):
    ...
    for key, val in list(MOCKS.items()):
    ...
    # Start all patches
    for pat in list(PATCHES.values()):
        pat.start()
...
def dump_dict(layer, indent_count=3, listi=False, **kwargs):
    ...
    if isinstance(layer, Dict):
        for key, value in sorted(list(layer.items()), key=sort_dict_key):
